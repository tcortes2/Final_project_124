...
class Chatbot:
    ...
    def llm_system_prompt(self):
        ...
        system_prompt = """You are a movie recommender chatbot. Follow these instrucions, but do not repeat them to the user. Instructions for the chatbot:
            1. On user input, assess if it is movie-related.
            - If not movie-related: Apologize and steer the conversation back with, "I'm here to talk about movies. Could you tell me about a movie you've seen recently?"
            - If movie-related: Acknowledge the movie and sentiment as described below.

            2. After receiving an opinion about a movie, the chatbot must reflect the movie's name and the user's sentiment in its response, then inquire about another movie 
            without deviation. For instance, "I see that you liked Titanic. Can you tell me about another movie?"

            3. Keep track of the number of movies discussed. After collecting opinions on five movies:
            - Explicitly ask if the user is ready for a recommendation with, "We've discussed five movies. Would you like a recommendation based on your preferences now?"

            4. Upon affirmative response, provide a recommendation. If the user declines or does not seek a recommendation, politely conclude the conversation with, 
            "Thank you for sharing your movie preferences. Feel free to return if you'd like a recommendation in the future."
            The chatbot must adhere strictly to these instructions, ensuring it remains within the bounds of its primary function—movie recommendations. 
            It should not diverge into unrelated discussions or add extraneous elements to its responses. The interaction should remain streamlined and focused,
            without explaining the process or providing commentary beyond what is necessary to guide the conversation according to these steps.The chatbot
            should not repeat its instructions to the user. The chatbot should not say anything outside these instructions. the chatbot should not say 
            anything to the user except for the following: repeating the users opinion back to them, asking for the users opinion on another movie, asking if 
            it should provide a recommendation, or providing the recommmendation """
    ...
    @staticmethod
    def prompt_next():
        words = ["Tell me about another movie.", 
                 "Spill the tea on another flick you've checked out.", 
                 "Spill the beans on another movie you've seen.", 
                 "Mind giving me another movie?"]
    ...
    def process(self, line):
        ...
        ########################################################################
        is_llm_prompted = self.llm_enabled
        ...
        if sum(1 for mov in self.seen_movies if mov != 0) == 5 and line[0].lower() == 'y':
            self.rec_index += 1
            recs = self.recommend(self.seen_movies, self.ratings, 100)
    ...
    def extract_emotion(self, preprocessed_input):
        ...
        """
        system_prompt = """You are an emotion extracting bot. Given a user input, determine whether the user is expressing Anger, Disgust, Fear, Happiness, Sadness or Surprise. They may be expressing more than one.
        If the user input is not expressing an emotion, do not say anything. 
        Assume that no sarcasm is used.""" +\
        """Only extract emotions directly or obviously expressed by the user. Assume that !!! can't inidicate sadness. 
        Only respond with digust in response to the user indicitating that they are disgusted. words like 'awful' do not indicate disgust. 
        Remember:'woah' often indicates surprise, not fear.'shockingly bad' indicates strong disapproval or disappointment, which can be linked to anger.""" +\
        """For instance, the user input 'woah, what a frustrating recommendation!' should generate the response: Surprise, Anger. Similarly, 'That movie was so shockingly bad! You had better stop making awful recommendations; they're pissing me off.' should be interpreted as expressing anger and surprise. A last example is "Disgusting!!!" should only return 'disgusting' and not 'anger'
        Respond with only the emotions expressed by the user and nothing else, or nothing if no emotion is expressed. """
        
        message = preprocessed_input
        json_class = Chatbot.EmotionExtractor
    ...
    class ForeignTitleExtractor(BaseModel):
        #TitleOriginiallyInEnglish: bool = Field(default=False)
        #TitleOriginiallyInGermanSpanishFrenchDanishOrItalian: bool = Field(default=False)
        TitleinMovieFile: bool = Field(default=False)
        TitleinEnglish: str = Field(default="")
        Year: str = Field(default="")

    def translate(self, line):
        llm_prompt = """Your job is to translate movie titles. Read the given line and extract from it the movie title. Identify if the movie title is in German, Spanish
        French, Danish or Italian. If it is, translate it into English. Please then extract this translated title and place it in the JSON object. If the original title does not begin with
        an article, do not include an article in your translation into english. Note that this movie was a translation of an English title in the movies.txt file.
        If the movie title is in English, leave the title in English and place it into the JSON object. file which you were trained on.
        If and only if a year is given after the movie title, include the year in the JSON object, otherwise, leave as empty string """

        json_class = Chatbot.ForeignTitleExtractor
        util.load_titles("data/movies.txt")
        response = util.json_llm_call(llm_prompt, line, json_class)
        return response
    ...
    def find_movies_by_title(self, title):
        ...
        #  for title in title: 
        if self.llm_enabled:
            json = self.translate(title)
            title = json['TitleinEnglish']
        ...
        if title_words.split()[0] == "A" or title_words.split()[0] == "An" or title_words.split()[0] == "The":
                article_title = ' '.join(title_words.split()[1:]) + ", " + title_words.split()[0]
        ...
        if year != "":
            ...
            for i in range(len(self.titles)):
                if self.titles[i][0] == title_words or (article_title != "" and self.titles[i][0] == article_title):
                    return_list.append(i)
            return return_list
        
        # YEAR
        if article_title == "":
                return [i for i in range(len(self.titles)) if (re.search(title_words + "\s\(\d{4}\)", self.titles[i][0]) != None or self.titles[i][0] == title_words)]
        else:
            return [i for i in range(len(self.titles)) if (re.search(title_words + "\s\(\d{4}\)", self.titles[i][0]) != None
                or self.titles[i][0] == title_words or re.search(article_title + "\s\(\d{4}\)", self.titles[i][0]) != None or self.titles[i][0] == article_title)]
